...
export default function DirectSignatureScreen({ route, navigation }: any) {
  ...
  const handleOK = async (signatureBase64: string) => {
    ...
    try {
      ...
      // Intentar validación biométrica de huella en móviles
      if (Platform.OS !== 'web') {
        ...
        if (hasHardware && isEnrolled) {
          ...
          Alert.alert('Aviso', 'No se detectó configuración de huella. El registro continuará sin validación.');
        }
      }
      ...
      if (Platform.OS === 'web') window.alert('Hubo un problema enviando la firma.');
      else Alert.alert('Error', 'Hubo un problema enviando la firma.');
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const finishProcess = () => {
    ...
    // Refresh and go to Home or MyContract
    if (employee) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        <Text style={styles.step}>FIRMA Y HUELLA OBLIGATORIA</Text>
        <Text style={styles.title}>Identidad Digital</Text>
        <Text style={styles.subtitle}>Dibuja tu firma digital. Al guardar, se te solicitará tu Huella Dactilar para validar tu identidad.</Text>
      </View>
        ...
        >
          ...
          >
            <Text style={styles.buttonText}>
              {isSubmitting ? 'ENVIANDO FIRMA...' : 'GUARDAR Y FINALIZAR'}
            </Text>
      ...
      <Modal visible={showSuccess} transparent={true} animationType="fade">
        <View style={styles.modalOverlay}>
          <LinearGradient colors={['#1e293b', '#0f172a']} style={styles.modalContent}>
            ...
            <Text style={styles.modalTitle}>¡Firma Registrada!</Text>
            <Text style={styles.modalText}>
              Tu firma ha sido enviada exitosamente a Recursos Humanos. Ahora podrán generar tu contrato.
            </Text>
